...
function formatDate($date)
{
    ...
    $timezone =  auth('customer')->user()->settings()->where('name','timezone')->first(['value']);
    if (!isset($setting->date_time_format) || !isset($setting->date_time_separator) || !isset($setting->timezone)) return $date;
        if (isset($timezone->value) && $timezone->value){
            $dateFormat = $timezone->value;
        }else{
            $dateFormat = $setting->timezone;
        }
    $date = str_replace([',', '_'], [', ', ' '], Carbon\Carbon::createFromTimeString($date)->timezone($dateFormat)->format(str_replace(' ', $setting->date_time_separator, $setting->date_time_format)));
    $dateTime->setTimezone(new DateTimeZone($dateFormat));
